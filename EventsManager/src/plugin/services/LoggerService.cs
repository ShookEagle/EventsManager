...
public class LoggerService(IEventsManager plugin) : ILoggerService
{
    ...
    public void Settings(CCSPlayerController executor, string setting, bool toggle, 
        Dictionary<string, object>? meta = null) =>
        Log($"{executor.PlayerName} {(toggle ? "enabled" : "disabled")} {setting}", LogType.SETTING, meta);
}